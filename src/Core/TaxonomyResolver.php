...
final class TaxonomyResolver
{
	...
	public static function fromPostTypeOrFallback(string $post_type): string {
		$taxonomy = self::fromPostType( $post_type );

		return taxonomy_exists( $taxonomy ) ? $taxonomy : PLATHIX_TAXONOMY;
	}

	public static function isPlathixTaxonomy(string $taxonomy): bool {
		return $taxonomy === PLATHIX_TAXONOMY || str_starts_with( $taxonomy, PLATHIX_TAX_PREFIX );
	}
}
