...
final class FoldersApi
{
	...
	public function deleteFolder(int $id, string $taxonomy, string $onChildren = FolderTreeService::DEFAULT_ON_CHILDREN): bool
	{
	...
	private function defaultFoldersLoader(string $taxonomy): array
	{
		...
		$countService = new FolderCountService($repository, Cache::make());

		return array_map(
			static fn (FolderDTO $folder): array => $folder->toArray(),
			$countService->getAllCached($taxonomy)
		);
	}
	...
	private function defaultFolderDeleter(int $id, string $taxonomy, string $onChildren): bool
	{
		...
		return $tree->deleteRecursive($id, $taxonomy, $onChildren);
	}
	...
	private function defaultFoldersReorderer(array $items, string $taxonomy): array
	{
		...
		foreach ( $items as $item ) {
			...
			$ordered = $tree->setOrder($id, $position, $taxonomy);
