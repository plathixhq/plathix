...
final class ImportExportApi
{
	...
	/** @var \Closure(array<string, mixed>, string[]|null): array{imported:int, taxonomies:int, skipped:int, errors:int, error_details:list<array{taxonomy: string, code: string, message: string}>} */
	...
	private function defaultImportStarter(string $adapter, string $postType): ImportJobDTO|\WP_Error
	{
		...
		$dispatchReason = (new RateLimiter(Cache::make()))->canDispatchHeavyJob(
		...
		$result = (new ImportManager())->startImport($adapter, $postType, get_current_user_id());
	...
	{
		return (new StructureExporter())->exportPayload();
	}
	...
	{
		return (new StructureExporter())->exportPayload($taxonomies);
	}
