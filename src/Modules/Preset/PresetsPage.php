...
final class PresetsPage
{
	...
	public function register(): void {
		add_action( 'admin_menu', [ $this, 'addPage' ], 11 );
	...
	public function enqueueScripts(): void {
		...
		$asset = \Plathix\Infrastructure\AssetManifest::read( 'js/admin-ui/preset.asset.php' );
		...
		AdminLayout::renderPage( self::PAGE_SLUG, function (): void {
			...
			if ( $tag_filter !== '' ) {
				...
				function (array $preset) use ($tag_filter): bool {
					$tags = array_map( [ $this, 'normalizeTag' ], (array) ( $preset['tags'] ?? [] ) );
			...
			$all_tags      = $this->collectTags( $all_presets_full );
...
?>
		...
		<div class="plathix-presets-tags">
			...
			<?php foreach ( $all_tags as $tag_slug => $tag_label ) :
				$isActive = $tag_slug === strtolower( $tag_filter );
				$url = $isActive ? $page_url : add_query_arg( 'plathix_tag', rawurlencode( $tag_slug ), $page_url );
				?>
				<a href="<?php echo esc_url( $url ); ?>"
				   class="plathix-presets-tag <?php echo $isActive ? 'is-active' : ''; ?>">
		...
		<div id="plathix-presets-no-results" class="plathix-presets__no-results" style="display:none;">
			...
			<?php $this->renderUploadModal( $upload_nonce ); ?>
			<?php
		} );
	}
	...
	private function renderCard(array $preset): void {
		$id          = (int) ( $preset['id'] ?? 0 );
				...
				<?php if ( ! empty( $tags ) ) : ?>
					<div class="plathix-preset-card__tags">
						<?php foreach ( $tags as $tag ) :
							$tag_slug = $this->normalizeTag( (string) $tag );
							if ( $tag_slug === '' ) {
								continue;
							}
							$tag_label = $this->displayTag( (string) $tag );
							$tag_url  = add_query_arg( 'plathix_tag', rawurlencode( $tag_slug ), $page_url );
							$isActive = $tag_slug === $tag_search;
							?>
							<a href="<?php echo esc_url( $tag_url ); ?>"
							   class="plathix-badge plathix-preset-card__tag <?php echo $isActive ? 'plathix-badge--ok' : 'plathix-badge--neutral'; ?>">
								<?php echo esc_html( $tag_label ); ?>
							</a>
						<?php endforeach; ?>
					</div>
	...
	private function renderUploadModal(string $nonce): void {
		?>
	...
	private function renderScratchButton(): void {
		$nonce             = wp_create_nonce( self::SCRATCH_ACTION );
		$taxonomy          = \Plathix\Core\Taxonomy::taxonomyForPostType( 'attachment' );
		$all_terms         = ( new \Plathix\Core\FolderRepository() )->getAll( $taxonomy );
		$systemSlugs      = \Plathix\Core\FolderRepository::systemSlugs();
	 ...
	 */
	private function collectTags(array $presets): array {
		$all = [];
		foreach ( $presets as $preset ) {
			foreach ( (array) ( $preset['tags'] ?? [] ) as $tag ) {
				$tag_slug = $this->normalizeTag( (string) $tag );
				if ( $tag_slug !== '' ) {
					$all[ $tag_slug ] = $this->displayTag( (string) $tag );
				}
			}
		}
		ksort( $all );
	...
	private function displayTag(string $tag): string {
		$tag = trim( $tag );
		$tag = ltrim( $tag, "# \t\n\r\0\x0B" );
		return $tag;
	}
