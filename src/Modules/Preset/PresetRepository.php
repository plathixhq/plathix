...
final class PresetRepository
{
	...
	public function create(array $record): int|\WP_Error {
		...
		$normalized['slug'] = $this->uniqueSlug($normalized['slug']);
		...
		for ( $attempt = 0; $attempt < 3; $attempt++ ) {
			$inserted = $wpdb->insert( PresetSchema::tableName(), $normalized, $formats ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- INSERT into the plugin's own preset table; nothing to cache on a write

			if ( $inserted !== false ) {
				return (int) ( $wpdb->insert_id ?? 0 );
			}

			if ( $this->findBySlug($normalized['slug']) === null ) {
				break;
			}
	 ...
	 * @return int|\WP_Error
	 */
	public function upsertBySlug(array $record): int|\WP_Error {
		$existing = $this->findBySlug( (string) ($record['slug'] ?? ''));
	...
	public function update(int $id, array $record): bool|\WP_Error {
		...
		unset($merged['id']);
		$normalized = $this->normalizeRecord($merged, false);

		$slug_owner = $this->findBySlug($normalized['slug']);
	...
	public function findLastApplied(): ?array {
		...
		$sql = $wpdb->prepare(
			'SELECT * FROM ' . PresetSchema::tableName() . ' WHERE blog_id = %d AND last_applied_at IS NOT NULL ORDER BY last_applied_at DESC, id DESC LIMIT 1', // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- custom preset table (PresetSchema::tableName(), prefix+const); blog_id bound via %d; single-row dashboard read; caching not worthwhile for a rare admin-only lookup. %i not used: same false-Plugin-Check-WARNING reason as findOneBy() above (verified on stand, see class docblock at the top of this file).
	...
	public function uniqueSlug(string $slug): string {
		$slug = sanitize_key($slug);
		if ( $slug === '' || $this->findBySlug($slug) === null ) {
			return $slug;
		}
	...
	public function list(array $filters = []): array {
		...
		$sql = $wpdb->prepare(
			'SELECT * FROM ' . PresetSchema::tableName() . ' WHERE ' . implode(' AND ', $where) . ' ORDER BY title ASC, id ASC', // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- table name via PresetSchema::tableName() (prefix+const); placeholders come from interpolated $where clauses (each carries %s/%d); values bound via ...$params; not injectable. %i not used for the table name: same false-Plugin-Check-WARNING reason as findOneBy()/findLastApplied() above (verified on stand, see class docblock).
			...$params
		);
	 ...
	 */
	private function normalizeRecord(array $record, bool $include_created_at = true): array {
		...
		return [
			...
			'last_applied_at'   => $this->normalizeNullableDatetime($record['last_applied_at'] ?? null),
		];
	}
	...
	private function hydrateRow(array $row): array {
		return [
			...
			'structure'         => $this->decodeJsonArray($row['structure_json'] ?? '[]'),
