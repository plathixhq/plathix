...
final class StructureImporter
{
	...
	public function import(array $payload, ?array $taxonomies = null): array {
		...
		foreach ( $entries as $entry ) {
			...
			$result = $this->importFolders( $folders, $taxonomy );
	 ...
	 */
	private function importFolders(array $folders, string $taxonomy): array {
		...
		while ( $pending !== [] && $pass++ < $passes ) {
			...
			foreach ( $pending as $folder ) {
				...
				$final_name = $this->resolveName( $name, $new_parent, $taxonomy );
	...
	private function resolveName(string $name, int $parent, string $taxonomy): string {
		$existing = get_terms( [
			'taxonomy'   => $taxonomy,
			'parent'     => $parent,
			'hide_empty' => false,
