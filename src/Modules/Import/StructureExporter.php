...
final class StructureExporter
{
	...
	public function __construct(
		...
		$this->meta_provider = \Closure::fromCallable(
			$meta_provider ?? static function (int $term_id, string $key): mixed {
				return ( new FolderRepository() )->getMeta( $term_id, $key );
			}
		);
	}

	/** @return string[] */
	public static function availableTaxonomies(): array {
		/** @var array<int, string> $merged */
		$merged = array_merge( [ (string) PLATHIX_TAXONOMY ], Taxonomy::getEnabledTaxonomies() );
	 ...
	 */
	public function exportPayload(?array $taxonomies = null): array {
		return [
			'plugin'       => 'plathix',
			'version'      => PLATHIX_VERSION,
			'generated_at' => gmdate( 'c' ),
			'site_url'     => home_url( '/' ),
			'taxonomies'   => $this->exportTaxonomies( $taxonomies ),
		];
	}
	 ...
	 * @param string[]|null $taxonomies null = all available
	 */
	public function exportJson(?array $taxonomies = null): string|false {
		return wp_json_encode(
			$this->exportPayload( $taxonomies ),
	 ...
	 */
	private function exportTaxonomies(?array $filter = null): array {
		$taxonomies = self::availableTaxonomies();
