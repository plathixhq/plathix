...
final class ImportToolsCard
{
	...
	public function render(): void
	{
		...
		foreach ( $adapters as $key => $available ) {
			$pending_checkpoints[ $key ] = $this->importManager->hasPendingCheckpoint( $key );
		}
		$this->renderCard( $adapters, $imported, $pending_checkpoints );
	}

	/**
	 * @param array<string, bool> $adapters
 ...
 ?>
				...
				<div class="plathix-import-grid">
					<?php foreach ( $adapters as $key => $available ) :
						...
						$hasPendingCheckpoint = ! empty( $pending_checkpoints[ $key ] );
						?>
