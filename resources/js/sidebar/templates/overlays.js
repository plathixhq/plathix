...
const contextMenuOverlay = () => `
    ...
    <div class="plathix-context-menu"
         ...
         x-effect="if (isOpen && isPositioned) $nextTick(() => $focusFirstIn($el))"
         @keydown.escape.window="close()"
         @keydown.tab="$trapFocus($el, $event)"
        ...
        <button type="button"
                ...
                @click="$store.plathix.canCreateChild($store.plathix.getFolderDepth(folder?.id)) ? createSubfolder() : (close(), $store.plathix.showAlert(${ escapeAttr( JSON.stringify( t( 'max_depth_reached', 'Maximum nesting depth reached' ) ) ) }))">
        ...
        <button type="button" role="menuitem" data-action="rename" x-show="$store.plathix.canManage && !folder?.isProtected" @click="rename()">
        ...
        <button type="button" role="menuitem" class="plathix-context-menu__danger" x-show="$store.plathix.canManage && !folder?.isProtected" @click="remove()">
    ...
    <div class="plathix-notifications" x-show="$store.plathix.notifications.length > 0">
        <template x-for="notif in $store.plathix.notifications" :key="notif.id">
            <div class="plathix-notification" :class="'plathix-notification--' + notif.type"
                ...
                <button type="button" class="plathix-notification__close" aria-label="${t('close_notification', 'Close notification')}" @click="$store.plathix.dismissNotification(notif.id)">&#215;</button>
            </div>
        </template>
    </div>
    </template>
...
const alertOverlay = () => `
    <template x-teleport="body">
    <div x-show="$store.plathix.alertMessage" class="plathix-alert__overlay" @click.self="$store.plathix.hideAlert()" @keydown.escape.window="$store.plathix.hideAlert()">
        <div class="plathix-alert__box" role="alertdialog" aria-modal="true" aria-labelledby="plathix-alert-title"
             x-effect="if ($store.plathix.alertMessage) $nextTick(() => $focusFirstIn($el))"
             @keydown.tab="$trapFocus($el, $event)">
            <p id="plathix-alert-title" x-text="$store.plathix.alertMessage"></p>
            <button type="button" class="button button-primary" @click="$store.plathix.hideAlert()">OK</button>
        </div>
    </div>
    </template>
`;
...
const bulkDeleteOverlay = () => `
    ...
    <div x-show="$store.plathix.deletingFoldersBulk" class="plathix-alert__overlay" @click.self="$store.plathix.hideBulkDeleteConfirm()" @keydown.escape.window="$store.plathix.hideBulkDeleteConfirm()">
        <div class="plathix-delete__box" role="alertdialog" aria-modal="true" aria-labelledby="plathix-bulk-delete-title"
             x-effect="if ($store.plathix.deletingFoldersBulk) $nextTick(() => $focusFirstIn($el))"
             @keydown.tab="$trapFocus($el, $event)">
            <p class="plathix-delete__title" id="plathix-bulk-delete-title">${t('bulk_delete_confirm_title', 'Move the selected folders to Trash? You can restore them later.')}</p>
...
const singleDeleteOverlay = () => `
    ...
    <div x-show="$store.plathix.deletingFolder" class="plathix-alert__overlay" @click.self="$store.plathix.hideDeleteConfirm()" @keydown.escape.window="$store.plathix.hideDeleteConfirm()">
        <div class="plathix-delete__box" role="alertdialog" aria-modal="true" aria-labelledby="plathix-single-delete-title"
             x-effect="if ($store.plathix.deletingFolder) $nextTick(() => $focusFirstIn($el))"
             @keydown.tab="$trapFocus($el, $event)">
            <p class="plathix-delete__title" id="plathix-single-delete-title">${t('delete_confirm_title', 'Move this folder to Trash? You can restore it later.')}</p>
