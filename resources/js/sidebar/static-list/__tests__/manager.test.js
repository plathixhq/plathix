...
describe('init — popstate', () => {
    ...
    it('resolves initial folderId to trashFolderId when the page URL has status=trash without attachment-filter', () => {
        Object.defineProperty(window, 'location', {
            value: { href: 'http://localhost/wp-admin/upload.php?status=trash', origin: 'http://localhost', assign: jest.fn() },
            writable: true,
            configurable: true,
        });
        ...
        expect(replaceUrl).toHaveBeenCalledWith('http://localhost/wp-admin/upload.php?status=trash', { plathixFolderId: 77 });
    });
});
...
describe('init — folder column links', () => {
    ...
    it('uses link.href directly without calling buildUrl again when href is present', () => {
        uploadInst.canHandle.mockReturnValue(true);
        uploadInst.buildUrl.mockReturnValue('http://localhost/wp-admin/upload.php?mode=list&paged=99');
