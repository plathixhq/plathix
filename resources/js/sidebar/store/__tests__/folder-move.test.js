...
describe('prevents concurrent state changes', () => {
    ...
    it('retries once and succeeds when the server responds with structure_locked', async () => {
        Api.moveFolderToSiblingOf
            .mockRejectedValueOnce({ code: 'structure_locked' })
            .mockResolvedValueOnce({});
        const store = makeStore({ folders: [{ id: 10, parentId: 3 }] });

        await store.moveFolderToSiblingOf(10, 7, 2);

        expect(Api.moveFolderToSiblingOf).toHaveBeenCalledTimes(2);
    });

    it('retries once and succeeds when the server responds with create_lock_failed', async () => {
        Api.moveFolderToSiblingOf
            .mockRejectedValueOnce({ code: 'create_lock_failed' })
            .mockResolvedValueOnce({});
        const store = makeStore({ folders: [{ id: 10, parentId: 3 }] });
    ...
    it('does not retry for any other code — withLoading() catches the error (no rethrow opt-in), one call only', async () => {
        Api.moveFolderToSiblingOf.mockRejectedValueOnce({ code: 'invalid_parent', message: 'Invalid parent folder.' });
        const store = makeStore({ folders: [{ id: 10, parentId: 3 }] });
