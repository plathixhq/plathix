...
Packaging the plugin into a distributable zip (`pnpm run build:release-zip`) additionally
requires [ripgrep](https://github.com/BurntSushi/ripgrep) (`rg`): the packaging script
verifies the archive contents with it and refuses to run rather than skip those checks
silently. Plain `pnpm run build` does not need it.
